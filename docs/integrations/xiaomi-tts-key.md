...
> Status: User Guide / Provider Dependent.
>
> 这是面向玩家的接入教程，不是 Agent System 或 Creative Engine 的架构事实。小米平台入口、免费额度、模型名称和限额可能变化；涉及上线说明或产品承诺前，需要重新核对供应商当前规则。

StoryPlay 的角色配音由小米 **MiMo-V2.5-TTS** 模型实时合成。按照本教程，你可以免费申请一个属于自己的 API Key，
填入 StoryPlay 后即可获得**更稳定的配音和更低的延迟**——而且这个 Key **只保存在你的浏览器里，绝不会经过我们的服务器**。
...
StoryPlay 默认使用一个**公共 Key** 为所有用户提供配音。小米对语音模型设有 **RPM（每分钟请求数）/ TPM（每分钟 Token 数）** 的调用限额，而公共 Key 的额度由全部用户共享。当同时在线的人较多时，公共 Key 很容易达到上限，表现为——
...
小米有**两类 Key**，分别对应不同的服务地址。在 StoryPlay 中填写时需要**先选择 Key 类型**——通过 Key 的前缀即可判断：`sk-` 是按量付费、`tp-` 是套餐，两者不能混用。
...
1. 回到 StoryPlay **首页**，在选项区下方点击 **「经常没声音？自带配音 Key（可选）」**。
2. 在弹窗里：
...
- 你的 API Key **只保存在当前浏览器的 `localStorage`**（键名 `storyplay:tts`）中。
...
**Q：我的 Key 安全吗？**
- 安全。Key 只存在你的本地浏览器中，只会发送至小米官方服务地址，不经过 StoryPlay 服务器。但请不要将 Key 公开发布或分享给他人。

---

有问题欢迎在 [GitHub Issues](https://github.com/wdiandian/storyplay/issues) 反馈。
